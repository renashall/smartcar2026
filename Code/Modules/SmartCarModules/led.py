...
LED_INT_Indexes = [0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80]
...
class Led:
    ...
    def turnOffLeds(self, leds):
        for led_num in leds:
            self.ledIndex(led_num, 0, 0, 0)        
    def ledIndex(self,LED_Num,R,G,B):
        if type(LED_Num) == str:
            index = LED_STR_Indexes[LED_Num]
        else:
            index = LED_INT_Indexes[LED_Num]
